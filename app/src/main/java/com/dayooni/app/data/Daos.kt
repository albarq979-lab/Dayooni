...
interface PersonDao {
    ...
    fun observeAll()
    @Query("SELECT * FROM people") suspend fun getAll(): List<PersonEntity>
    @Query("DELETE FROM people") suspend fun clearAll(): Flow<List<PersonEntity>>
...
interface DebtDao {
    ...
    fun observeAll()
    @Query("SELECT * FROM debts") suspend fun getAll(): List<DebtEntity>
    @Query("DELETE FROM debts") suspend fun clearAll(): Flow<List<DebtEntity>>
    ...
    @Insert suspend fun insert(debt: DebtEntity): Long
    @Update suspend fun update(debt: DebtEntity)
    @Delete suspend fun delete(debt: DebtEntity)
    ...
    @Query("SELECT COALESCE(SUM(amount - paidAmount), 0) FROM debts WHERE isCreditor = 0")
    fun observeTotalOwed(): Flow<Double>
    @Query("SELECT COALESCE(SUM(amount - paidAmount), 0) FROM debts WHERE isCreditor = 1")
...
interface PaymentDao {
    ...
    @Query("DELETE FROM payments") suspend fun clearAll()
...
interface NoteDao {
    ...
    fun observeAll()
    @Query("SELECT * FROM notes") suspend fun getAll(): List<NoteEntity>
    @Query("DELETE FROM notes") suspend fun clearAll(): Flow<List<NoteEntity>>
